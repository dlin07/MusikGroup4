...
absolutePath = os.path.join(os.getcwd(), 'lib', 'visual', 'arucoDisplay.png')
...
def transformCalibrationDisplay(mPos):
    global calibrationR
    global projectedPoints
    global realPoints
    global projectedToReal
    # 0, 1, 2, 3= tl, tr, br, bl projected quad
    # 4, 5, 6, 7 = tl, tr, br, bl real quad
    # scale actual triangle to half the size
    center = ((mPos[4][0] + mPos[6][0])/2, (mPos[4][1] + mPos[6][1])/2)
    displayCenter = ((mPos[0][0] + mPos[2][0])/2, (mPos[0][1] + mPos[2][1])/2)
    
    offset1 = ((mPos[4][0] - center[0])/2, (mPos[4][1] - center[1])/2)
    actualTlShrunk = (center[0] + offset1[0], center[1] + offset1[1])
    offset2 = ((mPos[5][0] - center[0])/2, (mPos[5][1] - center[1])/2)
    actualTrShrunk = (center[0] + offset2[0], center[1] + offset2[1])
    offset3 = ((mPos[6][0] - center[0])/2, (mPos[6][1] - center[1])/2)
    actualBrShrunk = (center[0] + offset3[0], center[1] + offset3[1])
    offset4 = ((mPos[7][0] - center[0])/2, (mPos[7][1] - center[1])/2)
    actualBlShrunk = (center[0] + offset4[0], center[1] + offset4[1])
    
    print(mPos)
    # print(calibration.shape)
    defaultPoints = np.array([[480, 270], [1440, 270], [1440, 810], [480, 810]]).astype(np.float32)
    projectedPoints = np.array([[mPos[0][0], mPos[0][1]],[mPos[1][0], mPos[1][1]],[mPos[2][0], mPos[2][1]], [mPos[3][0], mPos[3][1]]]).astype(np.float32)
    realPoints = np.array([[actualTlShrunk[0], actualTlShrunk[1]],[actualTrShrunk[0],actualTrShrunk[1]],[actualBrShrunk[0], actualBrShrunk[1]], [actualBlShrunk[0], actualBrShrunk[1]]]).astype(np.float32)
    # realTri = np.array([[mPos[3][0], mPos[3][1]],[mPos[4][0], mPos[4][1]],[mPos[5][0], mPos[5][1]]]).astype(np.float32)a 

    # print(projectedTri)
    # print(realTri)
    defaultToCamera = cv.getPerspectiveTransform(defaultPoints, projectedPoints)
    cameraToDefault = cv.getPerspectiveTransform(projectedPoints, defaultPoints)
    projectedToReal = cv.getPerspectiveTransform(projectedPoints, realPoints)

    warpedImg = cv.warpPerspective(calibrationR, defaultToCamera, (calibrationR.shape[1], calibrationR.shape[0]))
    warpedImg = cv.warpPerspective(warpedImg, projectedToReal, (warpedImg.shape[1], warpedImg.shape[0]))
    warpedImg = cv.warpPerspective(warpedImg, cameraToDefault, (warpedImg.shape[1], warpedImg.shape[0]))
...
while True:
    ...
    # if not (projectedPoints is None):
    #     cv.polylines(
    #         frame, [projectedPoints.astype(np.int32)], True, (0,255,255)
    #     )
    #     cv.polylines(
    #         frame, [realPoints.astype(np.int32)], True, (0,255,255)
    #     )
        # display the warped triangle and see if it matches
        # transformed = cv.transform(np.array([projectedPoints]), projectedToReal)
        # cv.polylines(
